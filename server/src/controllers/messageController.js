...
export const handleGetMessageByConverationId = async (req, res) => {
  ...
  const data = await messageService.getMessageByConverationId(conversationId);
  if (data) {
    return res.status(200).json(data);
  }
  return res.status(500).json({ message: "Internal Server Error" });
...
export const handleCountUnseenMessage = async (req, res) => {
  const { conversationId, userId } = req.body;
  if (!conversationId || !userId) {
    return res.status(400).json({ message: "missing param" });
  }
  const data = await messageService.countUnseenMessage(conversationId, userId);
  if (data >= 0) {
    return res.status(200).json(data);
  }
  return res.status(500).json({ message: "Internal Server Error" });
...
export const handleUpdateSeenMessages = async (req, res) => {
  const { conversationId, userId } = req.body;
  if (!conversationId || !userId) {
    return res.status(400).json({ message: "missing param" });
  }
  const data = await messageService.updateSeenMessages(conversationId, userId);
  if (data) {
    return res.status(200).json(data);
  }
  return res.status(500).json({ message: "Internal Server Error" });
...
export const uploadFileMessage = async (req, res) => {
  ...
  if (fileName) {
    return res
      .status(200)
      .json({ fileName: fileName + "/" + file_size.toFixed(2) + "mb" });
  }
...
export const handleUpdateReactionMessage = async (req, res) => {
  ...
  const data = await messageService.updateReactionMessage(messageId, emoji);
  if (data) {
    return res.status(200).json(data);
  }
  return res.status(500).json({ message: "Internal Server Error" });
